...
For scheduled/programmed workouts, setup uses candidate metadata or the configured setup default per exercise. For custom generated workouts, setup is modeled once as fixed preparation time. The production custom-workout model is:

`4 minutes + sum(working sets * 45 seconds + max(working sets - 1, 0) * prescribed rest seconds) + max(exercise count - 1, 0) * 60 seconds`

Rest is never counted after the final set of an exercise. The custom model intentionally does not count two minutes per set and then add rest, because that double-counts much of the user's time budget. Warm-up sets, cooldown, supersets, and buffer time are not modeled in this stage. Estimates are deterministic planning approximations, not exact predictions.
...
After downward fitting, duration also acts as a planning budget. Scheduled/programmed workouts use the utilization-based expansion policy. Custom generated workouts instead treat the requested duration as a hard fitting target with a five-minute tolerance: the engine starts from a valid minimum workout and iteratively applies the valid one-step change that most reduces distance to the target while preserving safety, hard volume caps, target-muscle coverage, and movement-pattern usefulness. It may add sets to selected target-relevant exercises or add a useful unused-family exercise at the practical minimum set count. It may remove optional sets when the minimum valid workout is over the target.

The preferred-volume multiplier is a soft expansion ceiling only; existing hard per-muscle maximums remain hard. Expansion stops with an explicit reason: target duration reached, too little meaningful budget remains, maximum useful volume reached, candidate saturation, or movement-pattern constraints. Custom workouts may leave spare time when no useful non-redundant volume fits; the engine never adds unsafe or meaningless work merely to fill time. For example, 75- and 90-minute requests may return the same plan when hard volume and/or movement-pattern usefulness prevents any accepted expansion. Exercise count is therefore an outcome of useful volume, coverage, rest, setup, transition cost, candidate availability, and safety limits, not an input chosen directly from requested minutes.
