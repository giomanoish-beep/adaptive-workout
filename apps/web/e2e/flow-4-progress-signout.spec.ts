...
test.describe('Flow 4 — Progress → profile goal edit → sign out', () => {
  ...
  test('verifies progress empty states, edits goal, and signs out', async ({
    page,
  }) => {
    // -- Open Progress tab --
    ...
    await expect(
      page.getByRole('button', { name: 'History', pressed: true }),
    ...
    const goalCard = page
      ...
      .filter({ hasText: 'Training goal' });
    await expect(goalCard.locator('.settings-card__value')).toContainText(
      'Recomposition',
    );
