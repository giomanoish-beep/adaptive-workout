...
describe('ai-glm-provider package', () => {
  it('exports the documented package name and provider identity', () => {
    expect(packageName).toBe('@adaptive-workout/ai-glm-provider');
    expect(glmProviderDefinition.providerId).toBe('glm');
    expect(glmProviderDefinition.supportedTasks).toContain('workout_intent_extraction');
  });
});
...
function okTransportResult(
  content: unknown,
  overrides: Partial<GlmTransportResult> = {},
): GlmTransportResult {
  return {
...
function fakeTransport(
  behavior:
    | { readonly kind: 'ok'; readonly result: GlmTransportResult }
    | { readonly kind: 'failure'; readonly failure: GlmTransportFailure }
    | { readonly kind: 'throw'; readonly error: unknown },
): GlmTransport & { readonly calls: GlmTransportCall[] } {
  const calls: GlmTransportCall[] = [];
  return {
    calls,
...
function registerEchoHandler(
  ...
  registerGlmTaskHandler('grounded_decision_explanation', {
    buildRequestPayload(request): GlmRequestPayload {
      return {
        model: 'glm-4-plus',
    ...
    parseTaskOutput(_request, response: GlmResponsePayload) {
      ...
      if (content === null) {
        return { status: 'failure', message: 'GLM returned no content.' };
      }
...
describe('GlmAiProvider execute', () => {
  it('returns a typed success result validated against the AI contract', async () => {
    ...
    const provider = new GlmAiProvider({ transport, clock: fixedClock });
...
describe('GlmAiProvider definition', () => {
  it('exposes the default model and advertises all structured tasks', () => {
    const transport = fakeTransport({ kind: 'ok', result: okTransportResult({ valid: true }) });
    const provider = new GlmAiProvider({ transport, clock: fixedClock });
    expect(provider.definition.modelId).toBe('glm-4-plus');
  ...
  it('allows a custom model id without changing the provider id', () => {
    ...
    expect(provider.definition.providerId).toBe('glm');
    expect(provider.definition.modelId).toBe('glm-4-air');
  });
});
